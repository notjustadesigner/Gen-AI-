...
function Filter({
  ...
  const countries = useMemo(() => {
    const set = new Set(
      data.map((row) => row["Country of residence"]?.trim()).filter(Boolean)
    );
  ...
  const educationLevels = useMemo(() => {
    const set = new Set(
      data.map((row) => row["Current education level"]?.trim()).filter(Boolean)
    );
    return ["All", ...Array.from(set).sort()];
  }, [data]);
  ...
  return (
    <div className="filter-container">
      ...
      <div className="description">
        ...
        />
        <p>Scroll, Hover and use Filters to explore the data</p>
      </div>
    </div>
  );
}
